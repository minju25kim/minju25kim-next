...
export type DatesWithPosts = { datesWithPosts?: Date[] };
...
function Calendar({
  ...
  const markedDays = datesWithPosts.map((d) => new Date(d))
  return (
    <DayPicker
      modifiers={{
        marked: markedDays
      ...
      modifiersClassNames={{
        marked: 'before:absolute before:bottom-0.5 before:w-1 before:h-1 before:bg-zinc-400 before:rounded-full before:content-[""]'
      }}
      fixedWeeks
      showOutsideDays
      ...
      classNames={{
        ...
        cell: cn(
          ...
          props.mode === "range"
            ...
            : "[&:has([aria-selected])]:rounded-md smooth-corners-md"
        ),
        day: cn(
          buttonVariants({ variant: "ghost" }),
          "h-10 sm:h-14 w-full p-0 font-normal aria-selected:opacity-100"
        ),
