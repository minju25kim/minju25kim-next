...
async function getBlogPosts(page: number = 1, sort: SortOption = 'latest') {
  try {
    ...
    const totalPosts = await db.collection("blog").countDocuments();
...
function SortSwitcher({ 
  ...
  return (
    <div className="flex items-center gap-2 bg-gray-100 p-1 rounded-lg">
      <Link
        href={`/blog?sort=latest&page=${currentPage}`}
      ...
      <Link
        href={`/blog?sort=oldest&page=${currentPage}`}
        className={`px-3 py-1.5 rounded-md transition-colors ${
          currentSort === 'oldest' 
            ? 'bg-white shadow-sm' 
            : 'hover:bg-gray-200'
...
export default async function BlogPage({
  ...
  const result = await getBlogPosts(currentPage, sort);
                  ...
                  {result.data.posts.map((post: MongoDBBlogPost) => (
                    <TableRow key={post._id.toString()}>
                      <TableCell className="font-medium">
                        <div>
                          <Link 
                            href={`/blog/${post.title.toLowerCase().replace(/\s+/g, '_')}`}
